...
class ReactCarousel extends React.Component {
    ...
    render() {
        let props = this.props, state = this.state, clz = props.className || "", gutterWidth = props.gutter || 0, pos = this._calculatePosition(), xPos = pos.x, style = {
            width: ((pos.itemWidth + gutterWidth) * props.items.length) + 'px',
            transform: 'translate(' + [xPos + 'px', '0px'] + ')'
        }, items = this.props.items.map((e, i) => {
            return (React.createElement(CarouselItem_1.CarouselItem, {label: e.label, key: e.key, onClick: this.onItemClick, index: i, width: pos.itemWidth, itemKey: e.key, isSelected: props.selIndex === i}));
        });
