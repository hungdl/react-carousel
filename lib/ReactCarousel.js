...
class ReactCarousel extends React.Component {
    ...
    render() {
        let props = this.props, state = this.state, clz = props.className || "", gutterWidth = props.gutter || 0, pos = this._calculatePosition(), itemRenderer = props.itemRenderer || null, xPos = pos.x, style = {
            ...
            return (itemRenderer && itemRenderer(e, i, this.onItemClick)) || (React.createElement(CarouselItem_1.CarouselItem, {label: e.label, key: e.key, onClick: this.onItemClick, index: i, width: pos.itemWidth, itemKey: e.key, isSelected: props.selIndex === i}));
        });
